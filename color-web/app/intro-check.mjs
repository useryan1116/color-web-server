...
const start=document.querySelector('#start'),uhd=document.querySelector('#uhd'),stop=document.querySelector('#stop'),status=document.querySelector('#status');
...
const run=async(tests)=>{
  if(running)return;running=true;start.disabled=true;uhd.disabled=true;stop.disabled=false;results.replaceChildren();
  try{
    for(const [index,[label,size,file]] of tests.entries()){
      if(!running)break;status.textContent=`正在測試第 ${index+1} / ${tests.length} 項`;
    ...
    status.textContent=running?'測試完成，請截圖結果':'測試已停止，可重新開始';
  }finally{running=false;start.disabled=false;uhd.disabled=false;stop.disabled=true;}
};
start.onclick=()=>run(cases);
uhd.onclick=()=>run([['標準直式 UHD・120 幀・HEVC','2160 × 3840','diagnostics/mobile-uhd-120-hevc.mp4']]);
